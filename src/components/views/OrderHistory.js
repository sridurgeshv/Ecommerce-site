  ...
  useEffect(() => {
    ...
    const fetchOrderHistory = async () => {
      try {
          ...
          orderFiles.map(async (file) => {
            ...
            if (orderKey.endsWith('.json')) {
              try {
                ...
                if (orderDetails.userId === profile.username || orderDetails.userData.email === profile.email) {
                  return orderDetails;
                } else {
                  return null; // Skip orders that don't belong to the current user
                }
              } catch (error) {
        ...
        console.log('Orders:', filteredOrders); // Add this console log for debugging

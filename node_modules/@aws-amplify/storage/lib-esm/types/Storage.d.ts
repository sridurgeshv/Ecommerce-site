...
import { StorageProvider, StorageProviderApi, AWSS3Provider, StorageProviderWithCopy, S3ProviderGetOuput, S3ProviderRemoveOutput, S3ProviderListOutput, S3ProviderCopyOutput, S3ProviderPutOutput, S3ProviderGetPropertiesOutput, StorageProviderWithGetProperties } from '../';
 ...
 * config.
 */
type StorageOperationConfig<T extends StorageProvider | StorageProviderWithCopy | StorageProviderWithGetProperties, U extends StorageProviderApi> = ReturnType<T['getProviderName']> extends 'AWSS3' ? LastParameter<AWSS3Provider[U]> : T extends StorageProviderWithGetProperties & StorageProviderWithCopy ? LastParameter<T[U]> & {
    provider: ReturnType<T['getProviderName']>;
} : T extends StorageProviderWithCopy ? LastParameter<T[Exclude<U, 'getProperties'>]> & {
    provider: ReturnType<T['getProviderName']>;
} : T extends StorageProviderWithGetProperties ? LastParameter<T[Exclude<U, 'copy'>]> & {
    provider: ReturnType<T['getProviderName']>;
